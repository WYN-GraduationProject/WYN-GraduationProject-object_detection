...
from infrastructure_layer.servicer.video_objectdetect import VideoObjectDetect
...
async def serve():
    port = GrpcManager().get_service_config('object_detect_service')[1]
    server = grpc.aio.server(futures.ThreadPoolExecutor(max_workers=10))
    add_VideoServiceServicer_to_server(VideoObjectDetect(), server)
    server.add_insecure_port('[::]:' + str(port))
    nacos_serverutils = NacosManager().get_server_utils("object-detect-service-gRPC", "0.0.0.0", port)
    # 注册服务
...
if __name__ == '__main__':
    logger.info("object_detect_service-gRPC服务启动...")
    asyncio.run(serve())
